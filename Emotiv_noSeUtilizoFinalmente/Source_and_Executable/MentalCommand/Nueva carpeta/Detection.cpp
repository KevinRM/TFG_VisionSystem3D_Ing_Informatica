...
Detection::Detection() {
	...
	eventState = IEE_EmoStateCreate();
	userID = 0;
	userCloudID = -1;
	enableLoger = false;
	userName = "kevinmrm";
	password = "123456789Ke";
	profileName = "Kevin Martin";
}

void Detection::startProgram() {
	bool correct = connect();
	printf("Llegue");
	/*if (correct) {
		Sleep(2000);
		system("CLS");
...
bool Detection::connect() {
	printf("======= CONECTANDO A LA NUBE =======\n");
    ...
    if (EC_GetUserDetail(&userCloudID) != EDK_OK) {
        printf("Error: Cannot get detail info, exit %s \n", userName);
        return false;
    }
